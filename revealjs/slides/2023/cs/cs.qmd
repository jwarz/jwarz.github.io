...
But only ~60 Mil distinct employments

```{r}
#| eval: false
# 4. Distinct4
    # Remove entries with different ecompany url, created, last_updated, deleted
    distinct(member_id, title, location, company_id, date_from_parsed, date_to_parsed, .keep_all = T) |>
    
    # Different start dates
    group_by(member_id, title, company_id) |> 
    filter(date_from_parsed == min(date_from_parsed)) |> 
    ungroup() |> 
    
    # Keep with end_date
    mutate(has_end_date = case_when(!is.na(date_to_parsed) ~ T,
                                    TRUE ~ F)) |> 
    group_by(member_id, title, company_id, date_from_parsed) |> 
    filter(if(!any(has_end_date)) TRUE else has_end_date) |> 
    ungroup() |> 
    select(-has_end_date) |>
    
    # Keep with location
    mutate(has_location = case_when(!is.na(location) ~ T,
                                    TRUE ~ F)) |> 
    group_by(member_id, title, company_id, date_from_parsed) |> 
    filter(if(!any(has_location)) TRUE else has_location) |> 
    ungroup() |> 
    select(-has_location) |>
    
    # One position at a (starting) time at a company (remove altered titles)
    group_by(member_id, company_id, date_from_parsed) |> 
...
me_dist4_prqt <- arrow::open_dataset("/me_dist4/") 
me_dist4_prqt |> glimpse()
```

```{r}
#| echo: false
# Load distinct member experiences
me_dist4_prqt <- arrow::open_dataset("~/02_diss/01_data_sources/01_coresignal/02_data/member_experience/me_dist/") 
me_dist4_prqt |> 
  glimpse()
```
...
::: {.callout-warning}
`deleted` not considered yet! And different Urls, locations and end dates... WIP
:::
...
me_wrangled_prqt    <- me_dist4_prqt |> 
...
me_joined_unnested_prqt <- open_dataset("/Users/jschwarz/02_diss/01_data_sources/01_coresignal/02_data/me_joined_unnested1.parquet")
...
```{r}
#| echo: false
me_joined_unnested_prqt <- open_dataset("/Users/jschwarz/02_diss/01_data_sources/01_coresignal/02_data/me_joined_unnested2.parquet")
me_joined_unnested_prqt |> 
  glimpse()
```
