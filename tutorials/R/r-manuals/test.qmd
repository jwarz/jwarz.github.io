...
award-winning[^1] S system, which was developed at Bell
...
### Suggestions to the reader {.unnumbered}
...
@sec-graphical-procedures, which can be read at almost any time and need
not wait until all the preceding sections have been digested.

 
# Introduction and preliminaries {#sec-introduction-and-preliminaries}

 
## The R environment {#sec-the-r-environment}
...
## Related software and documentation {#sec-related-software-and-documentation}
...
with Data* by John M. Chambers. See [References](References.html), for
precise references.
...
implementations in mind. See [What documentation exists for R](https://cran.r-project.org/doc/FAQ/R-FAQ.html#What-documentation-exists-for-R_003f)
in the R statistical system FAQ.

 
## R and statistics {#sec-r-and-statistics}
...
packages are given later (see @sec-packages).
...
## R and the window system {#sec-r-and-the-window-system}
...
## Using R interactively {#sec-using-r-interactively}
...
## 1.6 An introductory session {#sec-1.6-an-introductory-session}
...
@sec-appendix-a-a-sample-session.
 
## Getting help with functions and features {#sec-getting-help-with-functions-and-features}
...
## R commands, case sensitivity, etc. {#sec-r-commands-case-sensitivity-etc.}
...
use). Normally all alphanumeric symbols are allowed[^2]
(and in some countries this includes accented letters) plus `.` and
`_`', with the restriction that a name must start with '`.` or a
letter, and if it starts with `.` the second character must not be a
digit. Names are effectively unlimited in length.
...
Elementary commands can be grouped together into one compound expression
by braces (`{`' and '`}`). *Comments* can be put
almost[^3] anywhere, starting with a hashmark (`#`),
...
[^2]: For portable R code (including that to be used in R packages) only
A--Za--z0--9 should be used.

[^3]: **not** inside strings, nor within the argument list of a function
definition

[^4]: some of the consoles will not allow you to enter more, and amongst
those which do some will silently discard the excess and some will use
it as the start of the next line.
 
## Recall and correction of previous commands {#sec-recall-and-correction-of-previous-commands}
...
characters can be removed with the DEL key or added with the other keys.
More details are provided later: see @sec-appendix-c-the-command-line-editor
.
...
interactively with R. See [R and Emacs](https://cran.r-project.org/doc/FAQ/R-FAQ.html#R-and-Emacs)
 in The R statistical
system FAQ.
 
## Executing commands from or diverting output to a file {#sec-executing-commands-from-or-diverting-output-to-a-file}

If commands[^5] are stored in an external file, say
...
## Data permanency and removing objects {#sec-data-permanency-and-removing-objects}
...
called `.RData`[^6] in the current directory,
...
[^6]: The leading "dot" in this file name makes it *invisible* in normal
file listings in UNIX, and in default GUI file listings on macOS and
Windows.
 
# Simple manipulations; numbers and vectors {#sec-simple-manipulations-numbers-and-vectors}

 
## Vectors and assignment {#sec-vectors-and-assignment}
...
*and lost*[^8]. So now if we were to use the command
...
[^7]: With other than vector types of argument, such as `list` mode
arguments, the action of `c()` is rather different. See [Concatenating
lists](Concatenating-lists.html).

[^8]: Actually, it is still available as `.Last.value` before any other
statements are executed.
 
## Vector arithmetic {#sec-vector-arithmetic}
...
functions are available. `log`, `exp`, `sin`, `cos`, `tan`, `sqrt`, and
so on, all have their usual meaning. `max` and `min` select the largest
and smallest elements of a vector respectively. `range` is a function
whose value is a vector of length two, namely `c(min(x), max(x))`.
`length(x)` is the number of elements in `x`, `sum(x)` gives the total
of the elements in `x`, and `prod(x)` their product.
...
## Generating regular sequences {#sec-generating-regular-sequences}
...
`2*1:15` is the vector `c(2, 4, …, 28, 30)`. Put `n <- 10` and compare
...
`from=``value`{.variable} and `to=``value`{.variable}; thus `seq(1,30)`,
`seq(from=1, to=30)` and `seq(to=30, from=1)` are all the same as
`1:30`. The next two arguments to `seq()` may be named
`by=``value`{.variable} and `length=``value`{.variable}, which specify a
...
The fifth argument may be named `along=``vector`{.variable}, which is
normally used as the only argument to create the sequence
`1, 2, …, length(``vector`{.variable}`)`, or the empty sequence if the
...
and `!=` for inequality. In addition if `c1` and `c2` are logical
...
`sep=``string`{.variable}, which changes it to `string`{.variable},
possibly empty.
...
[^9]: `paste(..., collapse=``ss`{.variable}`)` joins the arguments into a
single character string putting `ss`{.variable} in between, e.g.,
`ss <- "|"`. There are more tools for character manipulation, see the
help for `sub` and `substring`.
 
## Index vectors; selecting and modifying subsets of a data set {#sec-index-vectors-selecting-and-modifying-subsets-of-a-data-set}
...
`vector[``index_vector`{.variable}`]` as having an arbitrary expression
...
## Other types of objects {#sec-other-types-of-objects}
...
-   *lists* are a general form of vector in which the various elements
    ...
    statistical computation. See @sec-lists.
...
-   *functions* are themselves objects in R which can be stored in the
    ...
    extend R. See @sec-writing-your-own-functions
    .
 
# Objects, their modes and attributes {#sec-objects-their-modes-and-attributes}

 
## Intrinsic attributes: mode and length {#sec-intrinsic-attributes:-mode-and-length}
...
namely *numeric*[^10], *complex*, *logical*,
*character* and *raw*.
...
`mode(``object`{.variable}`)` and `length(``object`{.variable}`)` can be
used to find out the mode and length of any defined structure
[^11].

Further properties of an object are usually provided by
`attributes(``object`{.variable}`)`, see [Getting and setting
attributes](Getting-and-setting-attributes.html). Because of this,
*mode* and *length* are also called "intrinsic attributes" of an object.
...
Now `d` and `z` are the same.[^12] There is a large
collection of functions of the form `as.``something`{.variable}`()` for
either coercion from one mode to another, or for investing an object
...
[^10]: *numeric* mode is actually an amalgam of two distinct modes, namely
*integer* and *double* precision, as explained in the manual.

[^11]: Note however that `length(``object`{.variable}`)` does not always
contain intrinsic useful information, e.g., when `object`{.variable} is
a function.

[^12]: In general, coercion from numeric to character and back again will
not be exactly reversible, because of roundoff errors in the character
representation.
 
## Changing the length of an object {#sec-changing-the-length-of-an-object}
...
function](The-scan_0028_0029-function.html).)
...
## Getting and setting attributes {#sec-getting-and-setting-attributes}

The function `attributes(``object`{.variable}`)` returns a list of all
the non-intrinsic attributes currently defined for that object. The
function `attr(``object`{.variable}`, ``name`{.variable}`)` can be used
to select a specific attribute. These functions are rarely used, except
in rather special circumstances when some new attribute is being created
...
either to associate a new attribute with `object`{.variable} or to
...
## The class of an object {#sec-the-class-of-an-object}
...
for an object-oriented style[^13] of programming in
...
orientation](Object-orientation.html), but only briefly.

[^13]: A different style using 'formal' or 'S4' classes is provided in
package `methods`.
 
# Ordered and unordered factors {#sec-ordered-and-unordered-factors}
...
[Contrasts](Contrasts.html)), we here look at a specific example.


## 4.1 A specific example {#sec-4.1-a-specific-example}

Suppose, for example, we have a sample of 30 tax accountants from all
the states and territories of Australia[^14] and
...
```
  ...
  ------------------------------------------------------------------------------------------------------------------ ---- --
  • [The function tapply() and ragged arrays](The-function-tapply_0028_0029-and-ragged-arrays.html){accesskey="1"}        
  • [Ordered factors](Ordered-factors.html){accesskey="2"}                                                                
  ------------------------------------------------------------------------------------------------------------------ ---- --

[^14]: Readers should note that there are eight states and territories in
...
## The function `tapply()` and ragged arrays {#sec-the-function-`tapply()`-and-ragged-arrays}
...
`statef`[^15], as if they were separate vector
...
(Writing functions will be considered later in @sec-writing-your-own-functions
. Note that R's a builtin
function `sd()` is something different.) After this assignment, the
...
[^15]: Note that `tapply()` also works in this case when its second
...
# Arrays and matrices {#sec-arrays-and-matrices}
...
array() function](The-array_0028_0029-function.html).
...
## Array indexing. Subsections of an array {#sec-array-indexing.-subsections-of-an-array}
...
## Index matrices {#sec-index-matrices}
...
## The `array()` function {#sec-the-`array()`-function}
...
rule](The-recycling-rule.html)) but `dim(h) <- c(3,4,2)` would signal an
...
### Mixed vector and array arithmetic. The recycling rule {#sec-mixed-vector-and-array-arithmetic.-the-recycling-rule}
...
## The outer product of two arrays {#sec-the-outer-product-of-two-arrays}
...
functions will be considered further in @sec-writing-your-own-functions
.

### An example: Determinants of 2 by 2 single-digit matrices {#sec-an-example:-determinants-of-2-by-2-single-digit-matrices}
...
execution](Loops-and-conditional-execution.html), is so inefficient as
to be impractical.

It is also perhaps surprising that about 1 in 20 such matrices is
singular.
 
## Generalized transpose of an array {#sec-generalized-transpose-of-an-array}

The function `aperm(a, perm)` may be used to permute an array, `a`. The
argument `perm` must be a permutation of the integers *{1, ..., k}*,
...
`perm[j]` becoming the new `j`-th dimension. The easiest way to think of
this operation is as a generalization of transposition for matrices.
...
## Matrix facilities {#sec-matrix-facilities}
...
The operator `%*%` is used for matrix multiplication. An *n* by *1* or
*1* by *n* matrix may of course be used as an *n*-vector if in the
...
[^16]: Note that `x %*% x` is ambiguous, as it could mean either x'x or x
...
### Linear equations and inversion {#sec-linear-equations-and-inversion}
...
like[^17] `x %*% solve(A,x)`, rather than computing
the inverse of `A`.

[^17]: Even better would be to form a matrix square root B with A = BB' and
...
### Singular value decomposition and determinants {#sec-singular-value-decomposition-and-determinants}
...
### Least squares fitting and the QR decomposition {#sec-least-squares-fitting-and-the-qr-decomposition}
...
(see [Linear models](Linear-models.html)) to `lsfit()` for regression
modelling.
...
discussed in [Statistical models in R](Statistical-models-in-R.html).
 
## Forming partitioned matrices, `cbind()` and `rbind()` {#sec-forming-partitioned-matrices-`cbind()`-and-`rbind()`}
...
## The concatenation function, `c()`, with arrays {#sec-the-concatenation-function-`c()`-with-arrays}
...
## Frequency tables from factors {#sec-frequency-tables-from-factors}
...
length factors. If there are *k* factor arguments, the result is a
*k*-way array of frequencies.
...
# Lists and data frames {#sec-lists-and-data-frames}
...
`[[``…`{.variable}`]]` is the operator used to select a single
element, whereas `[``…`{.variable}`]` is a general subscripting
operator. Thus the former is the *first object in the list* `Lst`, and
if it is a named list the name is *not* included. The latter is a
*sublist of the list `Lst` consisting of the first entry only. If it is
a named list, the names are transferred to the sublist.*
...
## Constructing and modifying lists {#sec-constructing-and-modifying-lists}
...
sets up a list `Lst` of *m* components using `object_1`{.variable}, ...,
...
### Concatenating lists {#sec-concatenating-lists}
...
-   Vector structures appearing as variables of the data frame must all
    have the *same length*, and matrix structures must all have the
    *same number of rows*.
...
### Making data frames {#sec-making-data-frames}
...
files](Reading-data-from-files.html).
 
### `attach()` and `detach()` {#sec-`attach()`-and-`detach()`}
...
### Working with data frames {#sec-working-with-data-frames}
...
### Attaching arbitrary lists {#sec-attaching-arbitrary-lists}
...
### Managing the search path {#sec-managing-the-search-path}
...
[^18]: See the on-line help for `autoload` for the meaning of the second
term.
 
# Reading data from files {#sec-reading-data-from-files}
...
as file editors or Perl[^19] to fit in with the
...
[^19]: Under UNIX, the utilities `sed` or`awk` can be used.
 
## The `read.table()` function {#sec-the-`read.table()`-function}
...
as character variables. This can be changed if necessary.
...
## The `scan()` function {#sec-the-`scan()`-function}
...
[Attaching arbitrary lists](Attaching-arbitrary-lists.html)).
...
## Accessing builtin datasets {#sec-accessing-builtin-datasets}
...
### 7.3.1 Loading data from other R packages {#sec-7.3.1-loading-data-from-other-r-packages}
...
## R as a set of statistical tables {#sec-r-as-a-set-of-statistical-tables}
...
`x` for `d``xxx`{.variable}, `q` for `p``xxx`{.variable}, `p` for
`q``xxx`{.variable} and `n` for `r``xxx`{.variable} (except for
`rhyper`, `rsignrank` and `rwilcox`, for which it is `nn`). In not quite
all cases is the non-centrality parameter `ncp` currently available: see
the on-line help for details.

The `p``xxx`{.variable} and `q``xxx`{.variable} functions all have
logical arguments `lower.tail` and `log.p` and the `d``xxx`{.variable}
...
`d``xxx`{.variable}`(..., log = TRUE)`), directly.
...
## Examining the distribution of a set of data {#sec-examining-the-distribution-of-a-set-of-data}
...
## One- and two-sample tests {#sec-one--and-two-sample-tests}
...
# Grouping, loops and conditional execution {#sec-grouping-loops-and-conditional-execution}

 
## Grouped expressions {#sec-grouped-expressions}
...
`{``expr_1`{.variable}`; ``…`{.variable}`; ``expr_m`{.variable}`}`, in
...
## Control statements {#sec-control-statements}

 
### Conditional execution: `if` statements {#sec-conditional-execution:-`if`-statements}
...
### Repetitive execution: `for` loops, `repeat` and `while` {#sec-repetitive-execution:-`for`-loops-`repeat`-and-`while`}
...
where `name`{.variable} is the loop variable. `expr_1`{.variable} is a
vector expression, (often a sequence like `1:20`), and
`expr_2`{.variable} is often a grouped expression with its
sub-expressions written in terms of the dummy *name*.
`expr_2`{.variable} is repeatedly evaluated as `name`{.variable} ranges
through the values in the vector result of `expr_1`{.variable}.
...
possibility here is to use `coplot()`,[^20] which
...
which are discussed in @sec-writing-your-own-functions
, and where more examples
will emerge.

[^20]: to be discussed later, or use `xyplot` from package
...
`name`{.variable}`(``expr_1`{.variable}`, ``expr_2`{.variable}`, …)` and
may occur anywhere a function call is legitimate.

 
## Simple examples {#sec-simple-examples}
...
more[^21]. It in turn uses the functions `qr()` and
...
[^21]: See also the methods described in [Statistical models in
R](Statistical-models-in-R.html)
 
## Defining new binary operators {#sec-defining-new-binary-operators}
...
## Named arguments and defaults {#sec-named-arguments-and-defaults}

As first noted in [Generating regular
sequences](Generating-regular-sequences.html), if arguments to called
functions are given in the "`name`{.variable}`=``object`{.variable}"
...
[The par() function](The-par_0028_0029-function.html), for more details
on the `par()` function.) This can be done by including an extra
...
## Assignments within functions {#sec-assignments-within-functions}
...
## More advanced examples {#sec-more-advanced-examples}

 
### Efficiency factors in block designs {#sec-efficiency-factors-in-block-designs}
...
### Dropping all names in a printed array {#sec-dropping-all-names-in-a-printed-array}
...
### Recursive numerical integration {#sec-recursive-numerical-integration}
...
top-level environment is reached without finding the symbol `total` then
that variable is created and assigned to there. For most users `<<-`
creates a global variable and assigns the value of the right hand side
to it[^22]. Only when `<<-` has been used in a
...
[^22]: In some sense this mimics the behavior in [S-PLUS]{.small} since in
[S-PLUS]{.small} this operator always creates or assigns to a global
variable.
 
## Customizing the environment {#sec-customizing-the-environment}
...
file named `.Rprofile`[^23] can be placed in
...
[^23]: So it is hidden under UNIX.
 
## Classes, generic functions and object orientation {#sec-classes-generic-functions-and-object-orientation}
...
A function named `gen`{.variable}`.``cl`{.variable} will be invoked by
the generic `gen`{.variable} for class `cl`{.variable}, so do not name
...
# Statistical models in R {#sec-statistical-models-in-r}
...
## Defining statistical models; formulae {#sec-defining-statistical-models-formulae}
...
*design matrix* and has columns *x_0, x_1, ..., x_p*, the determining
variables. Very often *x_0* will be a column of ones defining an
*intercept* term.
...
`M_1`{.variable}` %in% ``M_2`{.variable}

:   Similar to `M_1`{.variable}`:``M_2`{.variable}, but with a different
    coding.

`M_1`{.variable}` * ``M_2`{.variable}

:   `M_1`{.variable}` + ``M_2`{.variable}` + ``M_1`{.variable}`:``M_2`{.variable}.

`M_1`{.variable}` / ``M_2`{.variable}

:   `M_1`{.variable}` + ``M_2`{.variable}` %in% ``M_1`{.variable}.
...
## Generic functions for extracting model information {#sec-generic-functions-for-extracting-model-information}
...
`anova(``object_1`{.variable}`, ``object_2`{.variable}`)`

Compare a submodel with an outer model and produce an analysis of
variance table.

`coef(``object`{.variable}`)`

Extract the regression coefficient (matrix).

Long form: `coefficients(``object`{.variable}`)`.

`deviance(``object`{.variable}`)`
...
`plot(``object`{.variable}`)`

Produce four plots, showing residuals, fitted values and some
diagnostics.

`predict(``object`{.variable}`, newdata=``data.frame`{.variable}`)`
...
`residuals(``object`{.variable}`)`

Extract the (matrix of) residuals, weighted as appropriate.

Short form: `resid(``object`{.variable}`)`.
...
`vcov(``object`{.variable}`)`

Returns the variance-covariance matrix of the main parameters of a
fitted model object.
 
## Analysis of variance and model comparison {#sec-analysis-of-variance-and-model-comparison}

The model fitting function
`aov(``formula`{.variable}`, data=``data.frame`{.variable}`)` operates
at the simplest level in a very similar way to the function `lm()`, and
most of the generic functions listed in the table in [Generic functions
for extracting model
information](Generic-functions-for-extracting-model-information.html)
apply.
...
## Updating fitted models {#sec-updating-fitted-models}
...
`.`, only, can be used to stand for "the corresponding part of the old
...
`add1()`, `drop1()` and `step()`. The names of these give a good clue to
their purpose, but for full details see the on-line help.
 
## Generalized linear models {#sec-generalized-linear-models}
...
### The `glm()` function {#sec-the-`glm()`-function}
...
"Family Name" in the table in [Families](Families.html). Where there is
a choice of links, the name of the link may also be supplied with the
...
### The `gaussian` family {#sec-the-`gaussian`-family}
...
### The `binomial` family {#sec-the-`binomial`-family}
...
### Poisson models {#sec-poisson-models}
...
### Quasi-likelihood models {#sec-quasi-likelihood-models}
...
For example, consider fitting the non-linear regression y = theta_1 z_1
/ (z_2 - theta_2) + e which may be written alternatively as y = 1 /
...
## Nonlinear least squares and maximum likelihood models {#sec-nonlinear-least-squares-and-maximum-likelihood-models}
...
We seek the parameter values that minimize some index of lack-of-fit,
and they do this by trying out various parameter values iteratively.
Unlike linear regression for example, there is no guarantee that the
procedure will converge on satisfactory estimates. All the methods
require initial guesses about what parameter values to try, and
convergence may depend critically upon the quality of the starting
values.
...
## Some non-standard models {#sec-some-non-standard-models}
...
-   []{#index-Mixed-models} **Mixed models.** The recommended
    ...
    which some of the coefficients correspond to random effects. These
    functions make heavy use of formulae to specify the models.

-   []{#index-Local-approximating-regressions} **Local approximating
    regressions.** The `loess()` function fits a nonparametric
    regression by using a locally weighted regression. Such regressions
    are useful for highlighting a trend in messy data or for data
...
-   []{#index-Robust-regression} **Robust regression.** There are
...
-   []{#index-Additive-models} **Additive models.** This technique aims
    ...
    variable. Functions `avas` and `ace` in package
    [**acepack**](https://CRAN.R-project.org/package=acepack) and
    functions `bruto` and `mars` in package
    [**mda**](https://CRAN.R-project.org/package=mda) provide some
    examples of these techniques in user-contributed packages to R. An
    extension is **Generalized Additive Models**, implemented in
    user-contributed packages
    [**gam**](https://CRAN.R-project.org/package=gam) and
    [**mgcv**](https://CRAN.R-project.org/package=mgcv).

-   []{#index-Tree_002dbased-models} **Tree-based models.** Rather than
    ...
    model fitting function is `tree()`, but many other generic functions
    such as `plot()` and `text()` are well adapted to displaying the
    results of a tree-based model fit in a graphical way.
...
# Graphical procedures {#sec-graphical-procedures}
...
## High-level plotting commands {#sec-high-level-plotting-commands}
...
### The `plot()` function {#sec-the-`plot()`-function}
...
:   If `x`{.variable} and `y`{.variable} are vectors,
    `plot(``x`{.variable}`, ``y`{.variable}`)` produces a scatterplot of
    `y`{.variable} against `x`{.variable}. The same effect can be
    produced by supplying one argument (second form) as either a list
    containing two elements `x`{.variable} and `y`{.variable} or a
    two-column matrix.
...
### Displaying multivariate data {#sec-displaying-multivariate-data}
...
### Display graphics {#sec-display-graphics}
...
`hist(x)`\
`hist(x, nclass=``n`{.variable}`)`\
`hist(x, breaks=``b`{.variable}`, …)`
...
### Arguments to high-level plotting functions {#sec-arguments-to-high-level-plotting-functions}
...
`xlab=``string`{.variable}\
`ylab=``string`{.variable}
...
## Low-level plotting commands {#sec-low-level-plotting-commands}
...
:   []{#index-legend}
    ...
    `legend( , fill=``v`{.variable}`)`

    :   Colors for filled boxes

    `legend( , col=``v`{.variable}`)`

    :   Colors in which points or lines will be drawn

    `legend( , lty=``v`{.variable}`)`

    :   Line styles

    `legend( , lwd=``v`{.variable}`)`

    :   Line widths

    `legend( , pch=``v`{.variable}`)`
...
### Mathematical annotation {#sec-mathematical-annotation}
...
### Hershey vector fonts {#sec-hershey-vector-fonts}
...
## Interacting with graphics {#sec-interacting-with-graphics}
...
## Using graphics parameters {#sec-using-graphics-parameters}
...
### Permanent changes: The `par()` function {#sec-permanent-changes:-the-`par()`-function}
...
To save and restore *all* settable[^24] graphical
parameters use
...
[^24]: Some graphics parameters such as the size of the current device are
for information only.
 
### Temporary changes: Arguments to graphics functions {#sec-temporary-changes:-arguments-to-graphics-functions}
...
## Graphics parameters list {#sec-graphics-parameters-list}
...
### Graphical elements {#sec-graphical-elements}
...
### Axes and tick marks {#sec-axes-and-tick-marks}
...
:   Axis styles for the *x* and *y* axes, respectively. With styles
    ...
    amount of space at the edges.
 
### Figure margins {#sec-figure-margins}
...
### Multiple figure environment {#sec-multiple-figure-environment}
...
## Device drivers {#sec-device-drivers}
...
### PostScript diagrams for typeset documents {#sec-postscript-diagrams-for-typeset-documents}
...
### Multiple graphics devices {#sec-multiple-graphics-devices}
...
`dev.copy(device, …, which=``k`{.variable}`)`\
`dev.print(device, …, which=``k`{.variable}`)`
...
## Dynamic graphics {#sec-dynamic-graphics}
...
packages](https://cran.r-project.org/doc/manuals/R-exts.html#Creating-R-packages)
in Writing R Extensions. Here, we will describe them from a user's point
of view.
...
packages](https://cran.r-project.org/doc/manuals/R-admin.html#Installing-packages)
in R Installation and Administration) to install and update packages.
...
available on the search list (see [Namespaces](Namespaces.html)): these
will be included in the list given by
...
## Standard packages {#sec-standard-packages}
...
packages](../R-FAQ/Which-add-on-packages-exist-for-R_003f.html#which-add-on-packages-exist-for-r_003f)
in R FAQ, for a complete list.
 
## Contributed packages and CRAN {#sec-contributed-packages-and-cran}
...
(<https://www.bioconductor.org/>). The *R FAQ* contains a list of CRAN
...
# OS facilities {#sec-os-facilities}
...
## Files and directories {#sec-files-and-directories}
...
## System commands {#sec-system-commands}
...
## Compression and Archives {#sec-compression-and-archives}
...
very large extent done by *connections*, and the `file` function which
is used to open a connection to a file (or a URL) and is able to
...
## B.1 Invoking R from the command line {#sec-b.1-invoking-r-from-the-command-line}
...
-   Unless `--no-environ` was given, R searches for user and
    ...
    `R_HOME`{.variable}`/etc/Renviron.site` is used (if it
    exists). The user file is the one pointed to by the environment
    variable `R_ENVIRON_USER` if this is set; otherwise, files
    `.Renviron` in the current or in the user's home directory
    (in that order) are searched for. These files should contain lines
    of the form '`name`{.variable}`=``value`{.variable}'. (See
...
-   Then R searches for the site-wide startup profile unless the command
    ...
    `R_HOME`{.variable}`/etc/Rprofile.site` is used if this
    exists.
...
:   Specify the encoding to be assumed for input from the console or
    ...
    page. (`--encoding ``enc`{.variable} is also accepted.) The input is
    re-encoded to the locale R is running in and needs to be
...
:   (UNIX only) Turn off command-line editing via **readline**. This is
    ...
    Statistics") package. See @sec-appendix-c-the-command-line-editor
...
`--min-vsize=``N`{.variable}\
`--min-nsize=``N`{.variable}
...
for any other executable `cmd`{.variable} on the path or given by an
absolute filepath: this is useful to have the same environment as R or
the specific commands run under, for example to run `ldd` or `pdflatex`.
...
## B.2 Invoking R under Windows {#sec-b.2-invoking-r-under-windows}
...
'`name`{.variable}`=``value`{.variable}' pairs on the command
line.
...
## B.3 Invoking R under macOS {#sec-b.3-invoking-r-under-macos}
...
## B.4 Scripting with R {#sec-b.4-scripting-with-r}
...
# A sample session {#sec-appendix-a-a-sample-session}
...
`Start R appropriately for your platform (see `[`Invoking R`](Invoking-R.html)`).`
...
# Invoking R {#sec-appendix-b-invoking-r}

Users of R on Windows or macOS should read the OS-specific section
first, but command-line use is also supported.

 
# The command-line editor {#sec-appendix-c-the-command-line-editor}

## Preliminaries {#sec-c.1-preliminaries}
...
inbuilt command line editor: this is most common on macOS. You can find
out which version (if any) is available by running `extSoftVersion()` in
an R session.

It can be disabled (useful for usage with ESS [^25])
...
When using R with GNU[^26] **readline** capabilities,
...
such as [Control-m]{.kbd}, are obtained by holding the CTRL down while
you press the m key, and are written as [C-m]{.kbd} below. Meta
characters, such as [Meta-b]{.kbd}, are typed by holding down
META[^27] and pressing b, and written as [M-b]{.kbd}
in the following. If your terminal does not have a META key enabled, you
can still type Meta characters using two-character sequences starting
with [ESC]{.kbd}. Thus, to enter [M-b]{.kbd}, you could type ESCb. The
[ESC]{.kbd} character sequences are also allowed on terminals with real
Meta keys. Note that case is significant for Meta characters.

Some but not all versions[^28] of **readline** will
recognize resizing of the terminal window so this is best avoided.

## Editing actions {#sec-c.2-editing-actions}
...
ESC. (The default is Emacs-style, and only that is described here: for
*vi* mode see the **readline** documentation.)
...
## Command-line editor summary {#sec-c.3-command-line-editor-summary}

### Command recall and vertical motion {#sec-command-recall-and-vertical-motion}
...
### Horizontal motion of the cursor {#sec-horizontal-motion-of-the-cursor}
...
### Editing and re-submission {#sec-editing-and-re-submission}
...
[^25]: The 'Emacs Speaks Statistics' package; see the URL
<https://ESS.R-project.org/>

[^26]: It is possible to build R using an emulation of GNU **readline**,
such as one based on NetBSD's **editline** (also known as **libedit**),
in which case only a subset of the capabilities may be provided.

[^27]: On a PC keyboard this is usually the Alt key, occasionally the
'Windows' key. On a Mac keyboard normally no meta key is available.

[^28]: In particular, not versions 6.3 or later: this is worked around as
from R 3.4.0.
 
# Function and variable index {#sec-appendix-d-function-and-variable-index}
  ...
  !                                                                                                                                   
                    ...
                    [`!`](Logical-vectors.html#index-_0021):                                                                          [Logical vectors](Logical-vectors.html)

                    [`!=`](Logical-vectors.html#index-_0021_003d):                                                                    [Logical vectors](Logical-vectors.html)

                                                                                                                                      

  \%                                                                                                                                  

                    [`%*%`](Multiplication.html#index-_0025_002a_0025):                                                               [Multiplication](Multiplication.html)

                    [`%o%`](The-outer-product-of-two-arrays.html#index-_0025o_0025):                                                  [The outer product of two arrays](The-outer-product-of-two-arrays.html)

                                                                                                                                      

  &                                                                                                                                   

                    [`&`](Logical-vectors.html#index-_0026):                                                                          [Logical vectors](Logical-vectors.html)

                    [`&&`](Conditional-execution.html#index-_0026_0026):                                                              [Conditional execution](Conditional-execution.html)

                                                                                                                                      

  \*                                                                                                                                  

                    [`*`](Vector-arithmetic.html#index-_002a):                                                                        [Vector arithmetic](Vector-arithmetic.html)

                                                                                                                                      

  \+                                                                                                                                  

                    [`+`](Vector-arithmetic.html#index-):                                                                             [Vector arithmetic](Vector-arithmetic.html)
  ...
  .                                                                                                                                   
                    ...
                    [`.`](Updating-fitted-models.html#index-.):                                                                       [Updating fitted models](Updating-fitted-models.html)

                    [`.First`](Customizing-the-environment.html#index-.first):                                                        [Customizing the environment](Customizing-the-environment.html)

                    [`.Last`](Customizing-the-environment.html#index-.last):                                                          [Customizing the environment](Customizing-the-environment.html)

                                                                                                                                      

  /                                                                                                                                   

                    [`/`](Vector-arithmetic.html#index-_002f):                                                                        [Vector arithmetic](Vector-arithmetic.html)

                                                                                                                                      

  :                                                                                                                                   

                    [`:`](Generating-regular-sequences.html#index-_003a):                                                             [Generating regular sequences](Generating-regular-sequences.html)

                    [`::`](Namespaces.html#index-_003a_003a):                                                                         [Namespaces](Namespaces.html)

                    [`:::`](Namespaces.html#index-_003a_003a_003a):                                                                   [Namespaces](Namespaces.html)

                                                                                                                                      

  \<                                                                                                                                  

                    [`<`](Logical-vectors.html#index-_003c):                                                                          [Logical vectors](Logical-vectors.html)

                    [`<<-`](Scope.html#index-_003c_003c-):                                                                            [Scope](Scope.html)

                    [`<=`](Logical-vectors.html#index-_003c_003d):                                                                    [Logical vectors](Logical-vectors.html)

                                                                                                                                      

  =                                                                                                                                   

                    [`==`](Logical-vectors.html#index-_003d_003d):                                                                    [Logical vectors](Logical-vectors.html)

                                                                                                                                      

  \>                                                                                                                                  

                    [`>`](Logical-vectors.html#index-_003e):                                                                          [Logical vectors](Logical-vectors.html)

                    [`>=`](Logical-vectors.html#index-_003e_003d):                                                                    [Logical vectors](Logical-vectors.html)

                                                                                                                                      

  ?                                                                                                                                   

                    [`?`](Getting-help.html#index-_003f):                                                                             [Getting help](Getting-help.html)

                    [`??`](Getting-help.html#index-_003f_003f):                                                                       [Getting help](Getting-help.html)

                                                                                                                                      

  \^                                                                                                                                  

                    [`^`](Vector-arithmetic.html#index-_005e):                                                                        [Vector arithmetic](Vector-arithmetic.html)

                                                                                                                                      

  \|                                                                                                                                  

                    [`|`](Logical-vectors.html#index-_007c):                                                                          [Logical vectors](Logical-vectors.html)

                    [`||`](Conditional-execution.html#index-_007c_007c):                                                              [Conditional execution](Conditional-execution.html)

                                                                                                                                      

  \~                                                                                                                                  

                    [`~`](Formulae-for-statistical-models.html#index-_007e):                                                          [Formulae for statistical models](Formulae-for-statistical-models.html)

                                                                                                                                      

  A                                                                                                                                   

                    [`abline`](Low-level-plotting-commands.html#index-abline):                                                        [Low-level plotting commands](Low_002dlevel-plotting-commands.html)

                    [`ace`](Some-non-standard-models.html#index-ace):                                                                 [Some non-standard models](Some-non_002dstandard-models.html)

                    [`add1`](Updating-fitted-models.html#index-add1):                                                                 [Updating fitted models](Updating-fitted-models.html)

                    [`anova`](Generic-functions-for-extracting-model-information.html#index-anova):                                   [Generic functions for extracting model
                                                                                                                                      information](Generic-functions-for-extracting-model-information.html)

                    [`anova`](ANOVA-tables.html#index-anova-1):                                                                       [ANOVA tables](ANOVA-tables.html)

                    [`aov`](Analysis-of-variance-and-model-comparison.html#index-aov):                                                [Analysis of variance and model
                                                                                                                                      comparison](Analysis-of-variance-and-model-comparison.html)

                    [`aperm`](Generalized-transpose-of-an-array.html#index-aperm):                                                    [Generalized transpose of an
                                                                                                                                      array](Generalized-transpose-of-an-array.html)

                    [`array`](The-array_0028_0029-function.html#index-array):                                                         [The array() function](The-array_0028_0029-function.html)

                    [`as.data.frame`](Making-data-frames.html#index-as.data.frame):                                                   [Making data frames](Making-data-frames.html)

                    [`as.vector`](The-concatenation-function-c_0028_0029-with-arrays.html#index-as.vector):                           [The concatenation function c() with
                                                                                                                                      arrays](The-concatenation-function-c_0028_0029-with-arrays.html)

                    [`attach`](attach_0028_0029-and-detach_0028_0029.html#index-attach):                                              [attach() and detach()](attach_0028_0029-and-detach_0028_0029.html)

                    [`attr`](Getting-and-setting-attributes.html#index-attr):                                                         [Getting and setting attributes](Getting-and-setting-attributes.html)

                    [`attr`](Getting-and-setting-attributes.html#index-attr-1):                                                       [Getting and setting attributes](Getting-and-setting-attributes.html)

                    [`attributes`](Getting-and-setting-attributes.html#index-attributes):                                             [Getting and setting attributes](Getting-and-setting-attributes.html)

                    [`attributes`](Getting-and-setting-attributes.html#index-attributes-1):                                           [Getting and setting attributes](Getting-and-setting-attributes.html)

                    [`avas`](Some-non-standard-models.html#index-avas):                                                               [Some non-standard models](Some-non_002dstandard-models.html)

                    [`axis`](Low-level-plotting-commands.html#index-axis):                                                            [Low-level plotting commands](Low_002dlevel-plotting-commands.html)

                                                                                                                                      

  B                                                                                                                                   

                    [`boxplot`](One--and-two-sample-tests.html#index-boxplot):                                                        [One- and two-sample tests](One_002d-and-two_002dsample-tests.html)

                    [`break`](Repetitive-execution.html#index-break):                                                                 [Repetitive execution](Repetitive-execution.html)

                    [`bruto`](Some-non-standard-models.html#index-bruto):                                                             [Some non-standard models](Some-non_002dstandard-models.html)

                                                                                                                                      

  C                                                                                                                                   

                    [`c`](Vectors-and-assignment.html#index-c):                                                                       [Vectors and assignment](Vectors-and-assignment.html)

                    [`c`](Character-vectors.html#index-c-1):                                                                          [Character vectors](Character-vectors.html)

                    [`c`](The-concatenation-function-c_0028_0029-with-arrays.html#index-c-2):                                         [The concatenation function c() with
                                                                                                                                      arrays](The-concatenation-function-c_0028_0029-with-arrays.html)

                    [`c`](Concatenating-lists.html#index-c-3):                                                                        [Concatenating lists](Concatenating-lists.html)

                    [`C`](Contrasts.html#index-c):                                                                                    [Contrasts](Contrasts.html)

                    [`cbind`](Forming-partitioned-matrices.html#index-cbind):                                                         [Forming partitioned matrices](Forming-partitioned-matrices.html)

                    [`coef`](Generic-functions-for-extracting-model-information.html#index-coef):                                     [Generic functions for extracting model
                                                                                                                                      information](Generic-functions-for-extracting-model-information.html)

                    [`coefficients`](Generic-functions-for-extracting-model-information.html#index-coefficients):                     [Generic functions for extracting model
                                                                                                                                      information](Generic-functions-for-extracting-model-information.html)

                    [`contour`](Display-graphics.html#index-contour):                                                                 [Display graphics](Display-graphics.html)

                    [`contrasts`](Contrasts.html#index-contrasts):                                                                    [Contrasts](Contrasts.html)

                    [`coplot`](Displaying-multivariate-data.html#index-coplot):                                                       [Displaying multivariate data](Displaying-multivariate-data.html)

                    [`cos`](Vector-arithmetic.html#index-cos):                                                                        [Vector arithmetic](Vector-arithmetic.html)

                    [`crossprod`](Index-matrices.html#index-crossprod):                                                               [Index matrices](Index-matrices.html)

                    [`crossprod`](Multiplication.html#index-crossprod-1):                                                             [Multiplication](Multiplication.html)

                    [`cut`](Frequency-tables-from-factors.html#index-cut):                                                            [Frequency tables from factors](Frequency-tables-from-factors.html)

                                                                                                                                      

  D                                                                                                                                   

                    [`data`](Accessing-builtin-datasets.html#index-data):                                                             [Accessing builtin datasets](Accessing-builtin-datasets.html)

                    [`data.frame`](Making-data-frames.html#index-data.frame):                                                         [Making data frames](Making-data-frames.html)

                    [`density`](Examining-the-distribution-of-a-set-of-data.html#index-density):                                      [Examining the distribution of a set of
                                                                                                                                      data](Examining-the-distribution-of-a-set-of-data.html)

                    [`det`](Singular-value-decomposition-and-determinants.html#index-det):                                            [Singular value decomposition and
                                                                                                                                      determinants](Singular-value-decomposition-and-determinants.html)

                    [`detach`](attach_0028_0029-and-detach_0028_0029.html#index-detach):                                              [attach() and detach()](attach_0028_0029-and-detach_0028_0029.html)

                    [`determinant`](Singular-value-decomposition-and-determinants.html#index-determinant):                            [Singular value decomposition and
                                                                                                                                      determinants](Singular-value-decomposition-and-determinants.html)

                    [`dev.list`](Multiple-graphics-devices.html#index-dev.list):                                                      [Multiple graphics devices](Multiple-graphics-devices.html)

                    [`dev.next`](Multiple-graphics-devices.html#index-dev.next):                                                      [Multiple graphics devices](Multiple-graphics-devices.html)

                    [`dev.off`](Multiple-graphics-devices.html#index-dev.off):                                                        [Multiple graphics devices](Multiple-graphics-devices.html)

                    [`dev.prev`](Multiple-graphics-devices.html#index-dev.prev):                                                      [Multiple graphics devices](Multiple-graphics-devices.html)

                    [`dev.set`](Multiple-graphics-devices.html#index-dev.set):                                                        [Multiple graphics devices](Multiple-graphics-devices.html)

                    [`deviance`](Generic-functions-for-extracting-model-information.html#index-deviance):                             [Generic functions for extracting model
                                                                                                                                      information](Generic-functions-for-extracting-model-information.html)

                    [`diag`](Multiplication.html#index-diag):                                                                         [Multiplication](Multiplication.html)

                    [`dim`](Arrays.html#index-dim):                                                                                   [Arrays](Arrays.html)

                    [`dotchart`](Display-graphics.html#index-dotchart):                                                               [Display graphics](Display-graphics.html)

                    [`drop1`](Updating-fitted-models.html#index-drop1):                                                               [Updating fitted models](Updating-fitted-models.html)

                                                                                                                                      

  E                                                                                                                                   

                    [`ecdf`](Examining-the-distribution-of-a-set-of-data.html#index-ecdf):                                            [Examining the distribution of a set of
                                                                                                                                      data](Examining-the-distribution-of-a-set-of-data.html)

                    [`edit`](Editing-data.html#index-edit):                                                                           [Editing data](Editing-data.html)

                    [`eigen`](Eigenvalues-and-eigenvectors.html#index-eigen):                                                         [Eigenvalues and eigenvectors](Eigenvalues-and-eigenvectors.html)

                    [`else`](Conditional-execution.html#index-else):                                                                  [Conditional execution](Conditional-execution.html)

                    [`Error`](Analysis-of-variance-and-model-comparison.html#index-error):                                            [Analysis of variance and model
                                                                                                                                      comparison](Analysis-of-variance-and-model-comparison.html)

                    [`example`](Getting-help.html#index-example):                                                                     [Getting help](Getting-help.html)

                    [`exp`](Vector-arithmetic.html#index-exp):                                                                        [Vector arithmetic](Vector-arithmetic.html)

                                                                                                                                      

  F                                                                                                                                   

                    [`F`](Logical-vectors.html#index-f):                                                                              [Logical vectors](Logical-vectors.html)

                    [`factor`](Factors.html#index-factor):                                                                            @sec-ordered-and-unordered-factors

                    [`FALSE`](Logical-vectors.html#index-false):                                                                      [Logical vectors](Logical-vectors.html)

                    [`fivenum`](Examining-the-distribution-of-a-set-of-data.html#index-fivenum):                                      [Examining the distribution of a set of
                                                                                                                                      data](Examining-the-distribution-of-a-set-of-data.html)

                    [`for`](Repetitive-execution.html#index-for):                                                                     [Repetitive execution](Repetitive-execution.html)

                    [`formula`](Generic-functions-for-extracting-model-information.html#index-formula):                               [Generic functions for extracting model
                                                                                                                                      information](Generic-functions-for-extracting-model-information.html)

                    [`function`](Writing-your-own-functions.html#index-function):                                                     @sec-writing-your-own-functions 

                                                                                                                                      

  G                                                                                                                                   

                    [`getAnywhere`](Object-orientation.html#index-getanywhere):                                                       [Object orientation](Object-orientation.html)

                    [`getS3method`](Object-orientation.html#index-gets3method):                                                       [Object orientation](Object-orientation.html)

                    [`glm`](The-glm_0028_0029-function.html#index-glm):                                                               [The glm() function](The-glm_0028_0029-function.html)

                                                                                                                                      

  H                                                                                                                                   

                    [`help`](Getting-help.html#index-help):                                                                           [Getting help](Getting-help.html)

                    [`help`](Getting-help.html#index-help-1):                                                                         [Getting help](Getting-help.html)

                    [`help.search`](Getting-help.html#index-help.search):                                                             [Getting help](Getting-help.html)

                    [`help.start`](Getting-help.html#index-help.start):                                                               [Getting help](Getting-help.html)

                    [`hist`](Examining-the-distribution-of-a-set-of-data.html#index-hist):                                            [Examining the distribution of a set of
                                                                                                                                      data](Examining-the-distribution-of-a-set-of-data.html)

                    [`hist`](Display-graphics.html#index-hist-1):                                                                     [Display graphics](Display-graphics.html)

                                                                                                                                      

  I                                                                                                                                   

                    [`identify`](Interacting-with-graphics.html#index-identify):                                                      [Interacting with graphics](Interacting-with-graphics.html)

                    [`if`](Conditional-execution.html#index-if):                                                                      [Conditional execution](Conditional-execution.html)

                    [`if`](Conditional-execution.html#index-if-1):                                                                    [Conditional execution](Conditional-execution.html)

                    [`ifelse`](Conditional-execution.html#index-ifelse):                                                              [Conditional execution](Conditional-execution.html)

                    [`image`](Display-graphics.html#index-image):                                                                     [Display graphics](Display-graphics.html)

                    [`is.na`](Missing-values.html#index-is.na):                                                                       [Missing values](Missing-values.html)

                    [`is.nan`](Missing-values.html#index-is.nan):                                                                     [Missing values](Missing-values.html)

                                                                                                                                      

  J                                                                                                                                   

                    [`jpeg`](Device-drivers.html#index-jpeg):                                                                         [Device drivers](Device-drivers.html)

                                                                                                                                      

  K                                                                                                                                   

                    [`ks.test`](Examining-the-distribution-of-a-set-of-data.html#index-ks.test):                                      [Examining the distribution of a set of
                                                                                                                                      data](Examining-the-distribution-of-a-set-of-data.html)
  ...
  L                                                                                                                                   
                    ...
                    [`legend`](Low-level-plotting-commands.html#index-legend):                                                        [Low-level plotting commands](Low_002dlevel-plotting-commands.html)

                    [`length`](Vector-arithmetic.html#index-length):                                                                  [Vector arithmetic](Vector-arithmetic.html)

                    [`length`](The-intrinsic-attributes-mode-and-length.html#index-length-1):                                         [The intrinsic attributes mode and
                                                                                                                                      length](The-intrinsic-attributes-mode-and-length.html)

                    [`levels`](Factors.html#index-levels):                                                                            @sec-ordered-and-unordered-factors

                    [`lines`](Low-level-plotting-commands.html#index-lines):                                                          [Low-level plotting commands](Low_002dlevel-plotting-commands.html)

                    [`list`](Lists.html#index-list):                                                                                  @sec-lists

                    [`lm`](Linear-models.html#index-lm):                                                                              [Linear models](Linear-models.html)

                    [`lme`](Some-non-standard-models.html#index-lme):                                                                 [Some non-standard models](Some-non_002dstandard-models.html)

                    [`locator`](Interacting-with-graphics.html#index-locator):                                                        [Interacting with graphics](Interacting-with-graphics.html)

                    [`loess`](Some-non-standard-models.html#index-loess):                                                             [Some non-standard models](Some-non_002dstandard-models.html)

                    [`loess`](Some-non-standard-models.html#index-loess-1):                                                           [Some non-standard models](Some-non_002dstandard-models.html)

                    [`log`](Vector-arithmetic.html#index-log):                                                                        [Vector arithmetic](Vector-arithmetic.html)

                    [`lqs`](Some-non-standard-models.html#index-lqs):                                                                 [Some non-standard models](Some-non_002dstandard-models.html)

                    [`lsfit`](Least-squares-fitting-and-the-QR-decomposition.html#index-lsfit):                                       [Least squares fitting and the QR
                                                                                                                                      decomposition](Least-squares-fitting-and-the-QR-decomposition.html)

                                                                                                                                      

  M                                                                                                                                   

                    [`mars`](Some-non-standard-models.html#index-mars):                                                               [Some non-standard models](Some-non_002dstandard-models.html)

                    [`max`](Vector-arithmetic.html#index-max):                                                                        [Vector arithmetic](Vector-arithmetic.html)

                    [`mean`](Vector-arithmetic.html#index-mean):                                                                      [Vector arithmetic](Vector-arithmetic.html)

                    [`methods`](Object-orientation.html#index-methods):                                                               [Object orientation](Object-orientation.html)

                    [`min`](Vector-arithmetic.html#index-min):                                                                        [Vector arithmetic](Vector-arithmetic.html)

                    [`mode`](The-intrinsic-attributes-mode-and-length.html#index-mode):                                               [The intrinsic attributes mode and
                                                                                                                                      length](The-intrinsic-attributes-mode-and-length.html)
  ...
  N                                                                                                                                   
                    ...
                    [`NA`](Missing-values.html#index-na):                                                                             [Missing values](Missing-values.html)

                    [`NaN`](Missing-values.html#index-nan):                                                                           [Missing values](Missing-values.html)

                    [`ncol`](Matrix-facilities.html#index-ncol):                                                                      [Matrix facilities](Matrix-facilities.html)

                    [`next`](Repetitive-execution.html#index-next):                                                                   [Repetitive execution](Repetitive-execution.html)

                    [`nlm`](Nonlinear-least-squares-and-maximum-likelihood-models.html#index-nlm):                                    [Nonlinear least squares and maximum likelihood
                                                                                                                                      models](Nonlinear-least-squares-and-maximum-likelihood-models.html)

                    [`nlm`](Least-squares.html#index-nlm-1):                                                                          [Least squares](Least-squares.html)

                    [`nlm`](Maximum-likelihood.html#index-nlm-2):                                                                     [Maximum likelihood](Maximum-likelihood.html)

                    [`nlme`](Some-non-standard-models.html#index-nlme):                                                               [Some non-standard models](Some-non_002dstandard-models.html)

                    [`nlminb`](Nonlinear-least-squares-and-maximum-likelihood-models.html#index-nlminb):                              [Nonlinear least squares and maximum likelihood
                                                                                                                                      models](Nonlinear-least-squares-and-maximum-likelihood-models.html)

                    [`nrow`](Matrix-facilities.html#index-nrow):                                                                      [Matrix facilities](Matrix-facilities.html)

                                                                                                                                      

  O                                                                                                                                   

                    [`optim`](Nonlinear-least-squares-and-maximum-likelihood-models.html#index-optim):                                [Nonlinear least squares and maximum likelihood
                                                                                                                                      models](Nonlinear-least-squares-and-maximum-likelihood-models.html)

                    [`order`](Vector-arithmetic.html#index-order):                                                                    [Vector arithmetic](Vector-arithmetic.html)

                    [`ordered`](Ordered-factors.html#index-ordered):                                                                  [Ordered factors](Ordered-factors.html)

                    [`ordered`](Ordered-factors.html#index-ordered-1):                                                                [Ordered factors](Ordered-factors.html)

                    [`outer`](The-outer-product-of-two-arrays.html#index-outer):                                                      [The outer product of two arrays](The-outer-product-of-two-arrays.html)

                                                                                                                                      

  P                                                                                                                                   

                    [`pairs`](Displaying-multivariate-data.html#index-pairs):                                                         [Displaying multivariate data](Displaying-multivariate-data.html)

                    [`par`](The-par_0028_0029-function.html#index-par):                                                               [The par() function](The-par_0028_0029-function.html)

                    [`paste`](Character-vectors.html#index-paste):                                                                    [Character vectors](Character-vectors.html)

                    [`pdf`](Device-drivers.html#index-pdf):                                                                           [Device drivers](Device-drivers.html)

                    [`persp`](Display-graphics.html#index-persp):                                                                     [Display graphics](Display-graphics.html)

                    [`plot`](Generic-functions-for-extracting-model-information.html#index-plot):                                     [Generic functions for extracting model
                                                                                                                                      information](Generic-functions-for-extracting-model-information.html)

                    [`plot`](The-plot_0028_0029-function.html#index-plot-1):                                                          [The plot() function](The-plot_0028_0029-function.html)

                    [`pmax`](Vector-arithmetic.html#index-pmax):                                                                      [Vector arithmetic](Vector-arithmetic.html)

                    [`pmin`](Vector-arithmetic.html#index-pmin):                                                                      [Vector arithmetic](Vector-arithmetic.html)

                    [`png`](Device-drivers.html#index-png):                                                                           [Device drivers](Device-drivers.html)

                    [`points`](Low-level-plotting-commands.html#index-points):                                                        [Low-level plotting commands](Low_002dlevel-plotting-commands.html)

                    [`polygon`](Low-level-plotting-commands.html#index-polygon):                                                      [Low-level plotting commands](Low_002dlevel-plotting-commands.html)

                    [`postscript`](Device-drivers.html#index-postscript):                                                             [Device drivers](Device-drivers.html)

                    [`predict`](Generic-functions-for-extracting-model-information.html#index-predict):                               [Generic functions for extracting model
                                                                                                                                      information](Generic-functions-for-extracting-model-information.html)

                    [`print`](Generic-functions-for-extracting-model-information.html#index-print):                                   [Generic functions for extracting model
                                                                                                                                      information](Generic-functions-for-extracting-model-information.html)

                    [`prod`](Vector-arithmetic.html#index-prod):                                                                      [Vector arithmetic](Vector-arithmetic.html)

                                                                                                                                      

  Q                                                                                                                                   

                    [`qqline`](Examining-the-distribution-of-a-set-of-data.html#index-qqline):                                        [Examining the distribution of a set of
                                                                                                                                      data](Examining-the-distribution-of-a-set-of-data.html)

                    [`qqline`](Display-graphics.html#index-qqline-1):                                                                 [Display graphics](Display-graphics.html)

                    [`qqnorm`](Examining-the-distribution-of-a-set-of-data.html#index-qqnorm):                                        [Examining the distribution of a set of
                                                                                                                                      data](Examining-the-distribution-of-a-set-of-data.html)

                    [`qqnorm`](Display-graphics.html#index-qqnorm-1):                                                                 [Display graphics](Display-graphics.html)

                    [`qqplot`](Display-graphics.html#index-qqplot):                                                                   [Display graphics](Display-graphics.html)

                    [`qr`](Least-squares-fitting-and-the-QR-decomposition.html#index-qr):                                             [Least squares fitting and the QR
                                                                                                                                      decomposition](Least-squares-fitting-and-the-QR-decomposition.html)

                    [`quartz`](Device-drivers.html#index-quartz):                                                                     [Device drivers](Device-drivers.html)

                                                                                                                                      

  R                                                                                                                                   

                    [`range`](Vector-arithmetic.html#index-range):                                                                    [Vector arithmetic](Vector-arithmetic.html)

                    [`rbind`](Forming-partitioned-matrices.html#index-rbind):                                                         [Forming partitioned matrices](Forming-partitioned-matrices.html)

                    [`read.table`](The-read.table_0028_0029-function.html#index-read.table):                                          [The read.table() function](The-read_002etable_0028_0029-function.html)

                    [`rep`](Generating-regular-sequences.html#index-rep):                                                             [Generating regular sequences](Generating-regular-sequences.html)

                    [`repeat`](Repetitive-execution.html#index-repeat):                                                               [Repetitive execution](Repetitive-execution.html)

                    [`resid`](Generic-functions-for-extracting-model-information.html#index-resid):                                   [Generic functions for extracting model
                                                                                                                                      information](Generic-functions-for-extracting-model-information.html)

                    [`residuals`](Generic-functions-for-extracting-model-information.html#index-residuals):                           [Generic functions for extracting model
                                                                                                                                      information](Generic-functions-for-extracting-model-information.html)

                    [`rlm`](Some-non-standard-models.html#index-rlm):                                                                 [Some non-standard models](Some-non_002dstandard-models.html)

                    [`rm`](Data-permanency-and-removing-objects.html#index-rm):                                                       [Data permanency and removing
                                                                                                                                      objects](Data-permanency-and-removing-objects.html)

                                                                                                                                      

  S                                                                                                                                   

                    [`scan`](The-scan_0028_0029-function.html#index-scan):                                                            [The scan() function](The-scan_0028_0029-function.html)

                    [`sd`](The-function-tapply_0028_0029-and-ragged-arrays.html#index-sd):                                            [The function tapply() and ragged
                                                                                                                                      arrays](The-function-tapply_0028_0029-and-ragged-arrays.html)

                    [`search`](Managing-the-search-path.html#index-search):                                                           [Managing the search path](Managing-the-search-path.html)

                    [`seq`](Generating-regular-sequences.html#index-seq):                                                             [Generating regular sequences](Generating-regular-sequences.html)

                    [`shapiro.test`](Examining-the-distribution-of-a-set-of-data.html#index-shapiro.test):                            [Examining the distribution of a set of
                                                                                                                                      data](Examining-the-distribution-of-a-set-of-data.html)

                    [`sin`](Vector-arithmetic.html#index-sin):                                                                        [Vector arithmetic](Vector-arithmetic.html)

                    [`sink`](Executing-commands-from-or-diverting-output-to-a-file.html#index-sink):                                  [Executing commands from or diverting output to a
                                                                                                                                      file](Executing-commands-from-or-diverting-output-to-a-file.html)

                    [`solve`](Linear-equations-and-inversion.html#index-solve):                                                       [Linear equations and inversion](Linear-equations-and-inversion.html)

                    [`sort`](Vector-arithmetic.html#index-sort):                                                                      [Vector arithmetic](Vector-arithmetic.html)

                    [`source`](Executing-commands-from-or-diverting-output-to-a-file.html#index-source):                              [Executing commands from or diverting output to a
                                                                                                                                      file](Executing-commands-from-or-diverting-output-to-a-file.html)

                    [`split`](Repetitive-execution.html#index-split):                                                                 [Repetitive execution](Repetitive-execution.html)

                    [`sqrt`](Vector-arithmetic.html#index-sqrt):                                                                      [Vector arithmetic](Vector-arithmetic.html)

                    [`stem`](Examining-the-distribution-of-a-set-of-data.html#index-stem):                                            [Examining the distribution of a set of
                                                                                                                                      data](Examining-the-distribution-of-a-set-of-data.html)

                    [`step`](Generic-functions-for-extracting-model-information.html#index-step):                                     [Generic functions for extracting model
                                                                                                                                      information](Generic-functions-for-extracting-model-information.html)

                    [`step`](Updating-fitted-models.html#index-step-1):                                                               [Updating fitted models](Updating-fitted-models.html)

                    [`sum`](Vector-arithmetic.html#index-sum):                                                                        [Vector arithmetic](Vector-arithmetic.html)

                    [`summary`](Examining-the-distribution-of-a-set-of-data.html#index-summary):                                      [Examining the distribution of a set of
                                                                                                                                      data](Examining-the-distribution-of-a-set-of-data.html)

                    [`summary`](Generic-functions-for-extracting-model-information.html#index-summary-1):                             [Generic functions for extracting model
                                                                                                                                      information](Generic-functions-for-extracting-model-information.html)

                    [`svd`](Singular-value-decomposition-and-determinants.html#index-svd):                                            [Singular value decomposition and
                                                                                                                                      determinants](Singular-value-decomposition-and-determinants.html)
  ...
  T                                                                                                                                   
                    ...
                    [`T`](Logical-vectors.html#index-t):                                                                              [Logical vectors](Logical-vectors.html)

                    [`t`](Generalized-transpose-of-an-array.html#index-t):                                                            [Generalized transpose of an
                                                                                                                                      array](Generalized-transpose-of-an-array.html)

                    [`t.test`](One--and-two-sample-tests.html#index-t.test):                                                          [One- and two-sample tests](One_002d-and-two_002dsample-tests.html)

                    [`table`](Index-matrices.html#index-table):                                                                       [Index matrices](Index-matrices.html)

                    [`table`](Frequency-tables-from-factors.html#index-table-1):                                                      [Frequency tables from factors](Frequency-tables-from-factors.html)

                    [`tan`](Vector-arithmetic.html#index-tan):                                                                        [Vector arithmetic](Vector-arithmetic.html)

                    [`tapply`](The-function-tapply_0028_0029-and-ragged-arrays.html#index-tapply):                                    [The function tapply() and ragged
                                                                                                                                      arrays](The-function-tapply_0028_0029-and-ragged-arrays.html)

                    [`text`](Low-level-plotting-commands.html#index-text):                                                            [Low-level plotting commands](Low_002dlevel-plotting-commands.html)

                    [`title`](Low-level-plotting-commands.html#index-title):                                                          [Low-level plotting commands](Low_002dlevel-plotting-commands.html)

                    [`tree`](Some-non-standard-models.html#index-tree):                                                               [Some non-standard models](Some-non_002dstandard-models.html)

                    [`TRUE`](Logical-vectors.html#index-true):                                                                        [Logical vectors](Logical-vectors.html)

                                                                                                                                      

  U                                                                                                                                   

                    [`unclass`](The-class-of-an-object.html#index-unclass):                                                           [The class of an object](The-class-of-an-object.html)

                    [`update`](Updating-fitted-models.html#index-update):                                                             [Updating fitted models](Updating-fitted-models.html)

                                                                                                                                      

  V                                                                                                                                   

                    [`var`](Vector-arithmetic.html#index-var):                                                                        [Vector arithmetic](Vector-arithmetic.html)

                    [`var`](The-function-tapply_0028_0029-and-ragged-arrays.html#index-var-1):                                        [The function tapply() and ragged
                                                                                                                                      arrays](The-function-tapply_0028_0029-and-ragged-arrays.html)

                    [`var.test`](One--and-two-sample-tests.html#index-var.test):                                                      [One- and two-sample tests](One_002d-and-two_002dsample-tests.html)

                    [`vcov`](Generic-functions-for-extracting-model-information.html#index-vcov):                                     [Generic functions for extracting model
                                                                                                                                      information](Generic-functions-for-extracting-model-information.html)

                    [`vector`](Vectors-and-assignment.html#index-vector):                                                             [Vectors and assignment](Vectors-and-assignment.html)

                                                                                                                                      

  W                                                                                                                                   

                    [`while`](Repetitive-execution.html#index-while):                                                                 [Repetitive execution](Repetitive-execution.html)

                    [`wilcox.test`](One--and-two-sample-tests.html#index-wilcox.test):                                                [One- and two-sample tests](One_002d-and-two_002dsample-tests.html)

                    [`windows`](Device-drivers.html#index-windows):                                                                   [Device drivers](Device-drivers.html)

                                                                                                                                      

  X                                                                                                                                   

                    [`X11`](Device-drivers.html#index-x11):                                                                           [Device drivers](Device-drivers.html)
...
# Concept index {#sec-appendix-e-concept-index}
  ...
  A                                                                                                                                            
                    ...
                    [Accessing builtin datasets](Accessing-builtin-datasets.html#index-accessing-builtin-datasets):                            [Accessing builtin datasets](Accessing-builtin-datasets.html)

                    [Additive models](Some-non-standard-models.html#index-additive-models):                                                    [Some non-standard models](Some-non_002dstandard-models.html)

                    [Analysis of variance](Analysis-of-variance-and-model-comparison.html#index-analysis-of-variance):                         [Analysis of variance and model
                                                                                                                                               comparison](Analysis-of-variance-and-model-comparison.html)

                    [Arithmetic functions and operators](Vector-arithmetic.html#index-arithmetic-functions-and-operators):                     [Vector arithmetic](Vector-arithmetic.html)

                    [Arrays](Arrays.html#index-arrays):                                                                                        [Arrays](Arrays.html)

                    [Assignment](Vectors-and-assignment.html#index-assignment):                                                                [Vectors and assignment](Vectors-and-assignment.html)

                    [Attributes](Objects.html#index-attributes):                                                                               [Objects](Objects.html)

                                                                                                                                               

  B                                                                                                                                            

                    [Binary operators](Defining-new-binary-operators.html#index-binary-operators):                                             [Defining new binary operators](Defining-new-binary-operators.html)

                    [Box plots](One--and-two-sample-tests.html#index-box-plots):                                                               [One- and two-sample tests](One_002d-and-two_002dsample-tests.html)

                                                                                                                                               

  C                                                                                                                                            

                    [Character vectors](Character-vectors.html#index-character-vectors):                                                       [Character vectors](Character-vectors.html)

                    [Classes](The-class-of-an-object.html#index-classes):                                                                      [The class of an object](The-class-of-an-object.html)

                    [Classes](Object-orientation.html#index-classes-1):                                                                        [Object orientation](Object-orientation.html)

                    [Concatenating lists](Concatenating-lists.html#index-concatenating-lists):                                                 [Concatenating lists](Concatenating-lists.html)

                    [Contrasts](Contrasts.html#index-contrasts):                                                                               [Contrasts](Contrasts.html)

                    [Control statements](Control-statements.html#index-control-statements):                                                    [Control statements](Control-statements.html)

                    [CRAN](Contributed-packages-and-CRAN.html#index-cran):                                                                     [Contributed packages and CRAN](Contributed-packages-and-CRAN.html)

                    [Customizing the environment](Customizing-the-environment.html#index-customizing-the-environment):                         [Customizing the environment](Customizing-the-environment.html)

                                                                                                                                               

  D                                                                                                                                            

                    [Data frames](Data-frames.html#index-data-frames):                                                                         @sec-data-frames

                    [Default values](Named-arguments-and-defaults.html#index-default-values):                                                  [Named arguments and defaults](Named-arguments-and-defaults.html)

                    [Density estimation](Examining-the-distribution-of-a-set-of-data.html#index-density-estimation):                           [Examining the distribution of a set of
                                                                                                                                               data](Examining-the-distribution-of-a-set-of-data.html)

                    [Determinants](Singular-value-decomposition-and-determinants.html#index-determinants):                                     [Singular value decomposition and
                                                                                                                                               determinants](Singular-value-decomposition-and-determinants.html)

                    [Diverting input and                                                                                                       [Executing commands from or diverting output to a
                    output](Executing-commands-from-or-diverting-output-to-a-file.html#index-diverting-input-and-output):                      file](Executing-commands-from-or-diverting-output-to-a-file.html)

                    [Dynamic graphics](Dynamic-graphics.html#index-dynamic-graphics):                                                          [Dynamic graphics](Dynamic-graphics.html)

                                                                                                                                               

  E                                                                                                                                            

                    [Eigenvalues and eigenvectors](Eigenvalues-and-eigenvectors.html#index-eigenvalues-and-eigenvectors):                      [Eigenvalues and eigenvectors](Eigenvalues-and-eigenvectors.html)

                    [Empirical CDFs](Examining-the-distribution-of-a-set-of-data.html#index-empirical-cdfs):                                   [Examining the distribution of a set of
                                                                                                                                               data](Examining-the-distribution-of-a-set-of-data.html)
  ...
  F                                                                                                                                            
                    ...
                    [Factors](Factors.html#index-factors):                                                                                     @sec-ordered-and-unordered-factors

                    [Factors](Contrasts.html#index-factors-1):                                                                                 [Contrasts](Contrasts.html)

                    [Families](Families.html#index-families):                                                                                  [Families](Families.html)

                    [Formulae](Formulae-for-statistical-models.html#index-formulae):                                                           [Formulae for statistical
                                                                                                                                               models](Formulae-for-statistical-models.html)

                                                                                                                                               

  G                                                                                                                                            

                    [Generalized linear models](Generalized-linear-models.html#index-generalized-linear-models):                               [Generalized linear models](Generalized-linear-models.html)

                    [Generalized transpose of an                                                                                               [Generalized transpose of an
                    array](Generalized-transpose-of-an-array.html#index-generalized-transpose-of-an-array):                                    array](Generalized-transpose-of-an-array.html)

                    [Generic functions](Object-orientation.html#index-generic-functions):                                                      [Object orientation](Object-orientation.html)

                    [Graphics device drivers](Device-drivers.html#index-graphics-device-drivers):                                              [Device drivers](Device-drivers.html)

                    [Graphics parameters](The-par_0028_0029-function.html#index-graphics-parameters):                                          [The par() function](The-par_0028_0029-function.html)

                    [Grouped expressions](Grouped-expressions.html#index-grouped-expressions):                                                 [Grouped expressions](Grouped-expressions.html)

                                                                                                                                               

  I                                                                                                                                            

                    [Indexing of and by arrays](Array-indexing.html#index-indexing-of-and-by-arrays):                                          [Array indexing](Array-indexing.html)

                    [Indexing vectors](Index-vectors.html#index-indexing-vectors):                                                             [Index vectors](Index-vectors.html)

                                                                                                                                               

  K                                                                                                                                            

                    [Kolmogorov-Smirnov                                                                                                        [Examining the distribution of a set of
                    test](Examining-the-distribution-of-a-set-of-data.html#index-kolmogorov-smirnov-test):                                     data](Examining-the-distribution-of-a-set-of-data.html)

                                                                                                                                               

  L                                                                                                                                            

                    [Least squares                                                                                                             [Least squares fitting and the QR
                    fitting](Least-squares-fitting-and-the-QR-decomposition.html#index-least-squares-fitting):                                 decomposition](Least-squares-fitting-and-the-QR-decomposition.html)

                    [Linear equations](Linear-equations-and-inversion.html#index-linear-equations):                                            [Linear equations and inversion](Linear-equations-and-inversion.html)

                    [Linear models](Linear-models.html#index-linear-models):                                                                   [Linear models](Linear-models.html)

                    [Lists](Lists.html#index-lists):                                                                                           @sec-lists

                    [Local approximating regressions](Some-non-standard-models.html#index-local-approximating-regressions):                    [Some non-standard models](Some-non_002dstandard-models.html)

                    [Loops and conditional                                                                                                     [Loops and conditional
                    execution](Loops-and-conditional-execution.html#index-loops-and-conditional-execution):                                    execution](Loops-and-conditional-execution.html)

                                                                                                                                               

  M                                                                                                                                            

                    [Matrices](Arrays.html#index-matrices):                                                                                    [Arrays](Arrays.html)

                    [Matrix multiplication](Multiplication.html#index-matrix-multiplication):                                                  [Multiplication](Multiplication.html)

                    [Maximum likelihood](Maximum-likelihood.html#index-maximum-likelihood):                                                    [Maximum likelihood](Maximum-likelihood.html)

                    [Missing values](Missing-values.html#index-missing-values):                                                                [Missing values](Missing-values.html)

                    [Mixed models](Some-non-standard-models.html#index-mixed-models):                                                          [Some non-standard models](Some-non_002dstandard-models.html)

                                                                                                                                               

  N                                                                                                                                            

                    [Named arguments](Named-arguments-and-defaults.html#index-named-arguments):                                                [Named arguments and defaults](Named-arguments-and-defaults.html)

                    [Namespace](Namespaces.html#index-namespace):                                                                              [Namespaces](Namespaces.html)

                    [Nonlinear least                                                                                                           [Nonlinear least squares and maximum likelihood
                    squares](Nonlinear-least-squares-and-maximum-likelihood-models.html#index-nonlinear-least-squares):                        models](Nonlinear-least-squares-and-maximum-likelihood-models.html)

                                                                                                                                               

  O                                                                                                                                            

                    [Object orientation](Object-orientation.html#index-object-orientation):                                                    [Object orientation](Object-orientation.html)

                    [Objects](Objects.html#index-objects):                                                                                     [Objects](Objects.html)

                    [One- and two-sample tests](One--and-two-sample-tests.html#index-one--and-two-sample-tests):                               [One- and two-sample tests](One_002d-and-two_002dsample-tests.html)

                    [Ordered factors](Factors.html#index-ordered-factors):                                                                     @sec-ordered-and-unordered-factors

                    [Ordered factors](Contrasts.html#index-ordered-factors-1):                                                                 [Contrasts](Contrasts.html)

                    [Outer products of arrays](The-outer-product-of-two-arrays.html#index-outer-products-of-arrays):                           [The outer product of two
                                                                                                                                               arrays](The-outer-product-of-two-arrays.html)

                                                                                                                                               

  P                                                                                                                                            

                    [Packages](R-and-statistics.html#index-packages):                                                                          [R and statistics](R-and-statistics.html)

                    [Packages](Packages.html#index-packages-1):                                                                                @sec-packages

                    [Probability distributions](Probability-distributions.html#index-probability-distributions):                               [Probability distributions](Probability-distributions.html)

                                                                                                                                               

  Q                                                                                                                                            

                    [QR decomposition](Least-squares-fitting-and-the-QR-decomposition.html#index-qr-decomposition):                            [Least squares fitting and the QR
                                                                                                                                               decomposition](Least-squares-fitting-and-the-QR-decomposition.html)

                    [Quantile-quantile                                                                                                         [Examining the distribution of a set of
                    plots](Examining-the-distribution-of-a-set-of-data.html#index-quantile-quantile-plots):                                    data](Examining-the-distribution-of-a-set-of-data.html)

                                                                                                                                               

  R                                                                                                                                            

                    [Reading data from files](Reading-data-from-files.html#index-reading-data-from-files):                                     [Reading data from files](Reading-data-from-files.html)

                    [Recycling rule](Vector-arithmetic.html#index-recycling-rule):                                                             [Vector arithmetic](Vector-arithmetic.html)

                    [Recycling rule](The-recycling-rule.html#index-recycling-rule-1):                                                          [The recycling rule](The-recycling-rule.html)

                    [Regular sequences](Generating-regular-sequences.html#index-regular-sequences):                                            [Generating regular sequences](Generating-regular-sequences.html)

                    [Removing objects](Data-permanency-and-removing-objects.html#index-removing-objects):                                      [Data permanency and removing
                                                                                                                                               objects](Data-permanency-and-removing-objects.html)

                    [Robust regression](Some-non-standard-models.html#index-robust-regression):                                                [Some non-standard models](Some-non_002dstandard-models.html)

                                                                                                                                               

  S                                                                                                                                            

                    [Scope](Scope.html#index-scope):                                                                                           [Scope](Scope.html)

                    [Search path](Managing-the-search-path.html#index-search-path):                                                            [Managing the search path](Managing-the-search-path.html)

                    [Shapiro-Wilk test](Examining-the-distribution-of-a-set-of-data.html#index-shapiro-wilk-test):                             [Examining the distribution of a set of
                                                                                                                                               data](Examining-the-distribution-of-a-set-of-data.html)

                    [Singular value                                                                                                            [Singular value decomposition and
                    decomposition](Singular-value-decomposition-and-determinants.html#index-singular-value-decomposition):                     determinants](Singular-value-decomposition-and-determinants.html)

                    [Statistical models](Statistical-models-in-R.html#index-statistical-models):                                               [Statistical models in R](Statistical-models-in-R.html)

                    [Student's *t* test](One--and-two-sample-tests.html#index-student_0027s-t-test):                                           [One- and two-sample tests](One_002d-and-two_002dsample-tests.html)

                                                                                                                                               

  T                                                                                                                                            

                    [Tabulation](Frequency-tables-from-factors.html#index-tabulation):                                                         [Frequency tables from factors](Frequency-tables-from-factors.html)

                    [Tree-based models](Some-non-standard-models.html#index-tree-based-models):                                                [Some non-standard models](Some-non_002dstandard-models.html)

                                                                                                                                               

  U                                                                                                                                            

                    [Updating fitted models](Updating-fitted-models.html#index-updating-fitted-models):                                        [Updating fitted models](Updating-fitted-models.html)

                                                                                                                                               

  V                                                                                                                                            

                    [Vectors](Simple-manipulations-numbers-and-vectors.html#index-vectors):                                                    [Simple manipulations numbers and
                                                                                                                                               vectors](Simple-manipulations-numbers-and-vectors.html)

                                                                                                                                               

  W                                                                                                                                            

                    [Wilcoxon test](One--and-two-sample-tests.html#index-wilcoxon-test):                                                       [One- and two-sample tests](One_002d-and-two_002dsample-tests.html)

                    [Workspace](Data-permanency-and-removing-objects.html#index-workspace):                                                    [Data permanency and removing
                                                                                                                                               objects](Data-permanency-and-removing-objects.html)

                    [Writing functions](Writing-your-own-functions.html#index-writing-functions):                                              @sec-writing-your-own-functions 
...
# References {#sec-appendix-f-references}
