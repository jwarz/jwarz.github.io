...
```{r}
# data
inventory_parts_joined <- inventory_parts_joined |> select(set_num, part_num, color_id, quantity)
...
colors_joined <- batman_colors %>%
  full_join(star_wars_colors, by = "color_id", suffix = c("_batman", "_star_wars")) %>%
  # replace_na(list(total_batman = 0, total_star_wars = 0)) %>%
  mutate(across(ends_with("batman|star_wars"), ~replace_na(., 0))) %>%
  inner_join(colors, by = c("color_id" = "id")) %>%
  mutate(difference = fraction_batman - fraction_star_wars,
         total = total_batman + total_star_wars) %>%
  filter(total >= 200) %>%
  mutate(name = forcats::fct_reorder(name, difference)) 
```
...
# Data
color_palette <- readRDS("data/color_palette.rds")
